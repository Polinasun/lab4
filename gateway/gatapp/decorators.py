...
class TokenHeader():
    __slots__ = ('cache', 't_label', 't_type', 'requester', 'id', 'secret', 'url')

    def __init__(self, url, cache, app_id: str, app_secret: str, t_label: str = '<service>-token', t_type: str = 'Bearer'):
    ...
    def __request_new_token(self):
        response = requests.post(url = self.url, data = {'app_id': self.id, 'app_secret': self.secret})

        # json, code = self.requester.token(self.id, self.secret)

        if response.status_code == 200:
             self.__set_token(response.json()['token'])
